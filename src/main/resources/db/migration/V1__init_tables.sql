...
CREATE TABLE public.summary_feedbacks (
    ...
    comment TEXT,
    created_at timestamp(6) with time zone,
    CONSTRAINT unique_feedback_per_user UNIQUE (summary_id, user_id)
);
...
ALTER TABLE ONLY public.document_summaries
    ADD CONSTRAINT fk_document_summaries_document FOREIGN KEY (document_id) REFERENCES public.documents(id) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE ONLY public.document_summaries
    ADD CONSTRAINT fk_document_summaries_created_by FOREIGN KEY (created_by) REFERENCES public.users(id) ON DELETE SET NULL ON UPDATE CASCADE;

ALTER TABLE ONLY public.summary_feedbacks
    ADD CONSTRAINT fk_summary_feedbacks_summary FOREIGN KEY (summary_id) REFERENCES public.document_summaries(id) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE ONLY public.summary_feedbacks
    ADD CONSTRAINT fk_summary_feedbacks_user FOREIGN KEY (user_id) REFERENCES public.users(id) ON DELETE CASCADE ON UPDATE CASCADE;
...
CREATE INDEX idx_document_summaries_document ON public.document_summaries(document_id);