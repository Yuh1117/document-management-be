...
CREATE TABLE public.document_permissions (
    ...
    updated_by integer,
    user_id integer,
    CONSTRAINT document_permissions_permission_type_check CHECK (((permission_type)::text = ANY ((ARRAY['VIEW'::character varying, 'EDIT'::character varying])::text[])))
);
ALTER TABLE public.document_permissions OWNER TO postgres;
ALTER TABLE public.document_permissions ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
...
CREATE TABLE public.folder_permissions (
    ...
    updated_by integer,
    user_id integer,
    CONSTRAINT folder_permissions_permission_type_check CHECK (((permission_type)::text = ANY ((ARRAY['VIEW'::character varying, 'EDIT'::character varying])::text[])))
);
ALTER TABLE public.folder_permissions OWNER TO postgres;
ALTER TABLE public.folder_permissions ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME public.folder_permissions_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
